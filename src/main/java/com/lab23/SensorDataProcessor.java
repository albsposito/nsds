...
public class SensorDataProcessor {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < NO_SENSORS; i++) {
			sensors.add(sys.actorOf(TemperatureSensorActor.props(dispatcher), "t" + i));
		}

		// Dispatcher creates SensorProcessorActors ... //
		...
		// Re-configure dispatcher to use Load Balancing
		...
		ActorRef faultySensor = sys.actorOf(TemperatureSensorFaultyActor.props(dispatcher), "tFaulty");
		...
		System.out.println("Check after fault");
		for (int i = 0; i < SENSING_ROUNDS+1; i++) {
			for (ActorRef t : sensors) {
				t.tell(new GenerateMsg(), ActorRef.noSender());
			}
